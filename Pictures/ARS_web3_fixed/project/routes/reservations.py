...
def _register_arabic_font():
    """Register FreeSerif as Arabic-capable font for ReportLab"""
    try:
        from reportlab.pdfbase import pdfmetrics
        from reportlab.pdfbase.ttfonts import TTFont
        if 'ArabicFont' not in pdfmetrics.getRegisteredFontNames():
            # Try FreeSerif which has full Unicode/Arabic support
            candidates = [
...
def _ar_style(base_style, **kwargs):
    """Apply Arabic font to a ParagraphStyle"""
    from reportlab.lib.styles import ParagraphStyle
    font = _register_arabic_font()
    return ParagraphStyle(base_style, fontName=font, **kwargs)
...
def index():
    ...
    return render_template('reservations/index.html',
        ...
        status_ar=STATUS_AR, status_cls=STATUS_CLS,
        upcoming=upcoming,
...
def new():
    ...
    if request.method == 'POST':
        ...
        errors = []
        if not title:    errors.append('عنوان الحجز مطلوب')
        if not venue_id: errors.append('يرجى اختيار القاعة')
        ...
        try:
            ...
            if start_dt < datetime.now():
                errors.append('⚠️ لا يمكن إنشاء حجز بتاريخ في الماضي')
            if end_dt <= start_dt:
                errors.append('وقت الانتهاء يجب أن يكون بعد وقت البداية')
        except:
            errors.append('صيغة الوقت غير صحيحة')
...
def edit(res_id):
    ...
    if request.method == 'POST':
        ...
        try:
            start_dt = datetime.fromisoformat(start_str)
            end_dt   = datetime.fromisoformat(end_str)
            if end_dt <= start_dt: errors.append('وقت الانتهاء يجب أن يكون بعد وقت البداية')
        except:
            errors.append('صيغة الوقت غير صحيحة')
...
def export_single_pdf(res_id):
    ...
    try:
        ...
        try:
            ...
            for _p in [
                os.path.join(os.path.dirname(__file__), '..', 'maintenance_config.json'),
                os.path.join(os.getcwd(), 'maintenance_config.json'),
                'maintenance_config.json'
            ]:
                if os.path.exists(_p):
                    mcfg = _json.loads(open(_p).read())
                    break
        except: pass
        ...
        # ── Header ──
        _af = _register_arabic_font()
        center_style = ParagraphStyle('c', fontName=_af, fontSize=14, alignment=TA_CENTER,
                                      textColor=colors.HexColor('#1A555C'), spaceAfter=3)
        sub_style    = ParagraphStyle('s', fontName=_af, fontSize=11, alignment=TA_CENTER,
                                      textColor=colors.HexColor('#2E8B8F'), spaceAfter=2)
        small_style  = ParagraphStyle('sm', fontName=_af, fontSize=8, alignment=TA_CENTER,
                                      textColor=colors.grey, spaceAfter=6)

        if mcfg.get('report_header_title'):
            story.append(Paragraph(_ar_text(mcfg['report_header_title']), center_style))
        if mcfg.get('report_header_subtitle'):
            story.append(Paragraph(_ar_text(mcfg['report_header_subtitle']), sub_style))
        if mcfg.get('report_header_extra'):
            story.append(Paragraph(_ar_text(mcfg['report_header_extra']), small_style))
        if mcfg.get('report_header_title') or mcfg.get('report_header_subtitle'):
            story.append(HRFlowable(width='100%', thickness=1.5,
                                    color=colors.HexColor('#1A555C'), spaceAfter=10))

        # ── Title ──
        story.append(Paragraph(f'تفاصيل الحجز — {res.booking_number}',
            ParagraphStyle('t', fontName=_af, fontSize=13, textColor=colors.HexColor('#1A555C'),
                           spaceAfter=4, alignment=TA_CENTER)))
        story.append(Paragraph(f'Print Date / تاريخ الطباعة: {dt_date.today()}', small_style))
        ...
        data = [
            [_ar_text('رقم الحجز'),    res.booking_number or '—'],
            [_ar_text('العنوان'),       _ar_text(res.title or '—')],
            [_ar_text('المستخدم'),     _ar_text(res.user.full_name if res.user else '—')],
            [_ar_text('القاعة'),        _ar_text(res.venue.name if res.venue else '—')],
            [_ar_text('الموقع'),        _ar_text(res.venue.location.name if res.venue and res.venue.location else '—')],
            [_ar_text('تاريخ البدء'),  res.start_time.strftime('%Y-%m-%d %H:%M') if res.start_time else '—'],
            [_ar_text('تاريخ الانتهاء'), res.end_time.strftime('%Y-%m-%d %H:%M') if res.end_time else '—'],
            [_ar_text('الحالة'),        _ar_text(STATUS_AR.get(res.status, res.status))],
            [_ar_text('ملاحظات'),       _ar_text(res.requester_notes or '—')],
        ]
        t = Table(data, colWidths=[5*cm, 11*cm])
        t.setStyle(TableStyle([
            ('FONTSIZE',   (0,0), (-1,-1), 10),
            ('FONTNAME',   (0,0), (-1,-1), _af),
            ('ALIGN',      (0,0), (-1,-1), 'RIGHT'),
            ('VALIGN',     (0,0), (-1,-1), 'MIDDLE'),
            ('BACKGROUND', (0,0), (0,-1), light),
            ('FONTNAME',   (0,0), (0,-1), _af),
        ...
        # ── Footer ──
        footer_text = _ar_text(mcfg.get('report_header_footer') or mcfg.get('report_header_title', 'ARS — Applied Reservation System'))
        story.append(HRFlowable(width='100%', thickness=0.5,
                                color=colors.HexColor('#9ab3b8'), spaceAfter=4))
        story.append(Paragraph(footer_text,
            ParagraphStyle('f', fontName=_af, fontSize=8, alignment=TA_CENTER, textColor=colors.grey)))
...
def export_pdf():
    ...
    try:
        ...
        if mcfg.get('report_header_title'):
            story.append(Paragraph(mcfg['report_header_title'],
                ParagraphStyle('h', fontSize=14, textColor=colors.HexColor('#1A555C'), spaceAfter=2)))
        if mcfg.get('report_header_subtitle'):
            story.append(Paragraph(mcfg['report_header_subtitle'],
                ParagraphStyle('s', fontSize=11, textColor=colors.HexColor('#2E8B8F'), spaceAfter=4)))
        if mcfg.get('report_header_title'):
            story.append(HRFlowable(width='100%', thickness=1, color=colors.HexColor('#1A555C'), spaceAfter=6))

        story.append(Paragraph(f'ARS — Reservations Report  |  {dt_date.today()}',
            ParagraphStyle('t', fontSize=12, textColor=colors.HexColor('#1A555C'), spaceAfter=10)))
