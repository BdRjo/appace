...
CONFIG_EMAIL = 'email_config.json'
CONFIG_MAINT = 'maintenance_config.json'
...
def _load_ticker():
    try:
        ...
        return {
            ...
            'fg': '#F2C99A', 'bg': '', 'font': 'Tajawal',
            'size': 11, 'speed': 35, 'opacity': 0
        }
...
def maintenance():
    ...
    if request.method == 'POST':
        ...
        if action == 'add_ar':
            ...
            ticker['opacity'] = int(request.form.get('ticker_opacity', 100))
            _save_ticker(ticker)
            ...
            _save_maintenance(mcfg)
            syslog('SAVE_REPORT_HEADER', f"header updated by {current_user.username if current_user.is_authenticated else 'system'}")
        ...
        for log in recent:
            if log.user_id not in seen:
                ...
                users.append({
                    ...
                    'browser': (log.platform or '')[:40] if log.platform else '—',
...
def audit_log():
    ...
    q = db.query(SystemLog)
    if search: q = q.filter(SystemLog.action.ilike(f'%{search}%') | SystemLog.description.ilike(f'%{search}%'))
    if user_f and user_f.isdigit(): q = q.filter(SystemLog.user_id == int(user_f))
    items, total, total_pages = paginate(q.order_by(SystemLog.created_at.desc()), page, 30)
    users = db.query(User).filter_by(is_active=True).all()
    return render_template('admin/audit_log.html',
        logs=items, total=total, page=page, total_pages=total_pages,
        search=search, user_f=user_f, users=users)


# ── Current user connection info ──────────────────────────────────────────────
@admin_bp.route('/my-info')
@login_required
def my_info():
    ip = request.headers.get('X-Forwarded-For', request.remote_addr or '—').split(',')[0].strip()
    browser = request.user_agent.string[:80] if request.user_agent.string else '—'
    browser_name = request.user_agent.browser or '—'
    return jsonify({'ip': ip, 'browser': browser_name, 'agent': browser})
...
def settings():
    PROVIDERS = {
        'gmail':     {'label':'Gmail',                   'smtp':'smtp.gmail.com',          'port':587, 'help':'استخدم App Password من Google Account → Security → App passwords'},
        'office365': {'label':'Microsoft 365 / Outlook', 'smtp':'smtp.office365.com',       'port':587, 'help':'استخدم بريد Microsoft 365 وكلمة المرور أو App Password'},
        'outlook':   {'label':'Outlook.com (Hotmail)',   'smtp':'smtp-mail.outlook.com',    'port':587, 'help':'استخدم بريد Outlook.com وكلمة المرور'},
        'yahoo':     {'label':'Yahoo Mail',              'smtp':'smtp.mail.yahoo.com',      'port':587, 'help':'فعّل "Allow apps that use less secure sign in" أو استخدم App password'},
        'brevo':     {'label':'Brevo (Sendinblue)',       'smtp':'smtp-relay.brevo.com',     'port':587, 'help':'أنشئ حساباً مجانياً على brevo.com — اذهب إلى SMTP & API → Generate SMTP Key'},
        'custom':    {'label':'خادم مخصص',               'smtp':'',                          'port':587, 'help':'أدخل بيانات خادمك يدوياً'},
    }
...
def ticker_api():
    ...
    msgs  = ticker.get(key, ticker.get('ar', ['مرحباً بكم في نظام ARS']))
    if not msgs:
        msgs = ['مرحباً بكم في نظام ARS'] if lang == 'ar' else ['Welcome to ARS']
    return jsonify({
        'messages': msgs,
        'text':     ' ◆ '.join(msgs),
        'bg':       ticker.get('bg', ''),
