...
# We import Base lazily to avoid circular imports
try:
...
class HRSDepartment(_Base):
    ...
    parent     = relationship('HRSDepartment', remote_side='HRSDepartment.id', foreign_keys='HRSDepartment.parent_id')
    employees  = relationship('HRSEmployee', back_populates='department', foreign_keys='HRSEmployee.department_id')


class HRSPosition(_Base):
...
class HRSEmployee(_Base):
    ...
    department    = relationship('HRSDepartment', back_populates='employees', foreign_keys=[department_id])
    position      = relationship('HRSPosition', back_populates='employees')
    direct_manager = relationship('HRSEmployee', remote_side='HRSEmployee.id', foreign_keys=[direct_manager_id])
    education     = relationship('HRSEducation', back_populates='employee', cascade='all, delete-orphan')
    salary        = relationship('HRSSalary', back_populates='employee', uselist=False)
    leave_balances = relationship('HRSLeaveBalance', back_populates='employee', cascade='all, delete-orphan')
    leave_requests = relationship('HRSLeaveRequest', back_populates='employee', foreign_keys='HRSLeaveRequest.employee_id', cascade='all, delete-orphan')


class HRSEducation(_Base):
...
class HRSLeaveType(_Base):
    ...
    gender_specific     = Column(String(10), default='all')
...
class HRSLeaveBalance(_Base):
    __tablename__ = 'hrs_leave_balances'
    __table_args__ = (UniqueConstraint('employee_id', 'leave_type_id', 'year', name='uq_hrs_leave_balance'), {'extend_existing': True})
    id            = Column(Integer, primary_key=True)
    employee_id   = Column(Integer, ForeignKey('hrs_employees.id'), nullable=False)
...
class HRSLeaveRequest(_Base):
    ...
    created_at       = Column(DateTime, default=datetime.now)
    updated_at       = Column(DateTime, default=datetime.now, onupdate=datetime.now)
    employee         = relationship('HRSEmployee', back_populates='leave_requests', foreign_keys=[employee_id])
    leave_type       = relationship('HRSLeaveType', back_populates='requests')
    delegate         = relationship('HRSEmployee', foreign_keys=[delegate_id])
    approvals        = relationship('HRSLeaveApproval', back_populates='request', cascade='all, delete-orphan')


class HRSLeaveApproval(_Base):
...
class HRSSalary(_Base):
    __tablename__ = 'hrs_salaries'
    __table_args__ = {'extend_existing': True}
    id                  = Column(Integer, primary_key=True)
    employee_id         = Column(Integer, ForeignKey('hrs_employees.id'), nullable=False, unique=True)
    ...
    @property
    def net_salary(self):
        return self.gross_salary - (self.social_security + self.income_tax + self.other_deductions)


class HRSSalarySlip(_Base):
    __tablename__ = 'hrs_salary_slips'
    __table_args__ = (UniqueConstraint('employee_id', 'month', 'year', name='uq_hrs_salary_slip'), {'extend_existing': True})
    id                  = Column(Integer, primary_key=True)
    employee_id         = Column(Integer, ForeignKey('hrs_employees.id'), nullable=False)
    ...
    def gross_salary(self):
        return (self.basic_salary + self.housing_allowance + self.transport_allowance +
                self.food_allowance + self.phone_allowance + self.other_allowances + self.overtime_amount)

    @property
    def net_salary(self):
        return self.gross_salary - (self.social_security + self.income_tax + self.other_deductions + self.leave_deductions)
