...
@limiter.limit('10 per minute', error_message='محاولات كثيرة، انتظر دقيقة وحاول مجدداً')
def login():
...
def register():
    ...
    if request.method == 'POST':
        ...
        if email_ok:
            ...
            db.commit()
            current_app.logger.info(f"[EMAIL FAILED] activating user directly. Error: {email_err}")
...
def forgot_password():
    if request.method == 'POST':
        ...
        if step == '1':
            ...
            # ✅ تخزين في DB بدل session
            user.reset_code        = code
            user.reset_code_expiry = expires
            db.commit()
            ...
            if not user or not user.reset_code:
                flash_msg('انتهت الجلسة. أعد المحاولة.', 'danger')
                return render_template('auth/forgot_password.html', step=1)
            if user.reset_code_expiry and user.reset_code_expiry < datetime.now():
                user.reset_code = None; user.reset_code_expiry = None; db.commit()
